...
## Minutas (In Spanish)
1. 02/10/2019: Jorge nos habló sobre dos papers de la familia de BERT que están en revisión en ICLR: 1) [ELECTRA](https://openreview.net/forum?id=r1xMH1BtvB) y 2) [ALBERT](https://openreview.net/forum?id=H1eA7AEtvS). Idea interesante de ELECTRA: tener dos redes adversariales donde la primera genera oraciones corrompidas pero altamente probables (se reemplazan ciertas palabras por un muestreo de la salida de una softmax) y la segunda discrimina las palabras originales de las falsas (esto se hace con un sigmoide). Idea interesante de ALBERT: usar una embedding layer de menor dimensionalidad que se aumenta luego con una capa de proyección para reducir el número de parámetros de BERT y reemplazar la task de next sentence prediction por una que tome dos oraciones consecutivas, las desordene en algunos casos, y prediga si tienen el orden correcto o no.  
